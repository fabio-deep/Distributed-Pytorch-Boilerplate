...
def train(model, dataloaders, args):
    ...
    if args.optimiser == 'adam':
        ...
        optimiser = optim.SGD(model.parameters(),
            lr=args.learning_rate, weight_decay=args.weight_decay, momentum=0.9) #, nesterov=True)
    else:
        raise NotImplementedError('{} not setup.'.format(args.optimiser))

    '''lr schedulers'''
    if args.decay_steps > 0:
        lr_decay = optim.lr_scheduler.ExponentialLR(optimiser, gamma=args.decay_rate)
    elif args.decay_milestones[0] > 0:
        lr_decay = optim.lr_scheduler.MultiStepLR(optimiser,
            milestones=args.decay_milestones, gamma=args.decay_rate)
    else:
        lr_decay = optim.lr_scheduler.ReduceLROnPlateau(optimiser, factor=args.decay_rate,
            mode='max', patience=30, cooldown=20, min_lr=1e-6, verbose=True)
